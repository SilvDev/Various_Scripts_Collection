...
void ProcessLists()
{
	...
	{
		HTTPRequestHandle hRequest = Steam_CreateHTTPRequest(HTTPMethod_GET, sLink);
		Steam_SetHTTPRequestHeaderValue(hRequest, "Pragma", "no-cache");
		Steam_SetHTTPRequestHeaderValue(hRequest, "Cache-Control", "no-cache");
		Steam_SendHTTPRequest(hRequest, OnSteamHTTPComplete);
	}
	// REST IN PAWN
...
void OnSteamHTTPComplete(HTTPRequestHandle HTTPRequest, bool requestSuccessful, HTTPStatusCode statusCode, int main)
{
	...
	{
		Steam_WriteHTTPResponseBody(HTTPRequest, body);
	...
	{
		char sError[256];
		FormatEx(sError, sizeof(sError), "SteamTools error (status code %i). Request successful: %s", statusCode, requestSuccessful ? "True" : "False");
		LogError(sError);
	}
...
void DownloadMainList()
{
	...
	{
		HTTPRequestHandle hRequest = Steam_CreateHTTPRequest(HTTPMethod_GET, UPDATE_MAIN_LIST);
		Steam_SetHTTPRequestHeaderValue(hRequest, "Pragma", "no-cache");
		Steam_SetHTTPRequestHeaderValue(hRequest, "Cache-Control", "no-cache");
		Steam_SendHTTPRequest(hRequest, OnSteamHTTPComplete);
	}
