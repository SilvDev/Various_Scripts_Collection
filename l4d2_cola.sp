...
Action CmdColaTemp(int client, int args)
{
	if( !client )
	{
		ReplyToCommand(client, "[Cola] Command can only be used %s", IsDedicatedServer() ? "in game on a dedicated server." : "in chat on a Listen server.");
...
Action CmdColaList(int client, int args)
{
	...
	for( int i = 0; i < MAX_COLAS; i++ )
	{
		if( IsValidEntRef(g_iColas[i][0]) )
		{
			...
			else			PrintToConsole(client, "[Cola] %d) %f %f %f", i+1, vPos[0], vPos[1], vPos[2]);
		}
	}

	if( client )	PrintToChat(client, "%sTotal: %d.", CHAT_TAG, count);
	else			PrintToConsole(client, "[Cola] Total: %d.", count);
	return Plugin_Handled;
}
